    ...
    if (split_transform_flag) 
      {
        ...
        // P.L. blow 8 lines: silly define removed and put the real code in place for easier readebility 
        if( ret = hls_transform_tree( s, x0, y0, x0, y0, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size - 1, trafo_depth + 1, 0, cbf_cb, cbf_cr ) < 0 )                                    \
          return ret ; 
        if( ret = hls_transform_tree( s, x1, y0, x0, y0, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size - 1, trafo_depth + 1, 1, cbf_cb, cbf_cr ) < 0 )                                    \
          return ret ; 
        if( ret = hls_transform_tree( s, x0, y1, x0, y0, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size - 1, trafo_depth + 1, 2, cbf_cb, cbf_cr ) < 0 )                                    \
          return ret ; 
        if( ret = hls_transform_tree( s, x1, y1, x0, y0, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size - 1, trafo_depth + 1, 3, cbf_cb, cbf_cr ) < 0 )                                    \
          return ret ; 
      } 
    else 
      {
